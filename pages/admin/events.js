...
export default function AdminEvents() {
  ...
  async function submit(e) {
    ...
    if (res.ok) {
      ...
      setStatus('❌ ' + (json?.error || 'Error al crear evento'))
    }
  }
  ...
  const messageStyle = { marginTop: 24, padding: 16, borderRadius: 12, background: status?.includes('❌') ? '#fee2e2' : '#dcfce7', color: status?.includes('❌') ? '#991b1b' : '#166534', fontWeight: 500 }
