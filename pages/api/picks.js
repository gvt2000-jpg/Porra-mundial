...
<<<<<<< HEAD
import { PICKS_LOCK_LABEL, getPicksUnlockStatus } from '../../lib/picksLock'

export default async function handler(req, res) {
  if (req.method !== 'POST') return res.status(405).end()

  const lockStatus = await getPicksUnlockStatus(supabase)
  if (lockStatus.locked) {
    return res.status(403).json({ error: `Las predicciones estan cerradas desde el ${PICKS_LOCK_LABEL}.` })
  }
  ...
  try {
    ...
    if (existing.data && existing.data.length > 0) {
      return res.status(400).json({ error: 'Ya existe un Top10 enviado con este nombre. Usa otro nombre o elige otro nombre publico.' })
    }
    ...
    for (const p of picks) {
      ...
      if (!rank || rank < 1 || rank > 10) return res.status(400).json({ error: 'Cada pick debe tener una posicion entre 1 y 10.' })
      if (!p.team_id || typeof p.team_id !== 'string') return res.status(400).json({ error: 'Cada pick debe incluir un team_id valido.' })
    ...
    if (toInsert.length !== 10) return res.status(400).json({ error: 'Se requieren exactamente 10 picks unicos.' })
