...
import { saveLeaderboardSnapshot } from './leaderboard'
...
const WINNER_NEXT_STAGE_REACHED_FIELD = {
  round_of_32: 'reached_round_of_16',
  round_of_16: 'reached_quarter_final',
  quarter_final: 'reached_semi_final',
  semi_final: 'reached_final'
}

function markReachedStage(scores, match) {
  const reachedField = STAGE_REACHED_FIELD[match.stage]
  if (!reachedField) return
...
function markWinnerAdvanced(scores, match, winnerId) {
  const reachedField = WINNER_NEXT_STAGE_REACHED_FIELD[match.stage]
  if (!reachedField || !scores[winnerId]) return
  scores[winnerId][reachedField] = true
}
...
export async function recomputeTeamPoints(supabase) {
  ...
  const leaderboardSnapshotBefore = await saveLeaderboardSnapshot(supabase, 'before_recompute')
  ...
  const { data: redCards, error: eventError } = await supabase
    .from('match_events')
    .select('team_id, match_id, event_type')
    .eq('event_type', 'red_card')
  if (eventError) throw eventError

  const playedMatchIdsBeforeSync = new Set((matches || []).filter((match) => match.played).map((match) => match.id))
  const playedRedCards = (redCards || []).filter((event) => playedMatchIdsBeforeSync.has(event.match_id))

  await syncTournamentProgression(supabase, matches || [], { events: playedRedCards })
  ...
  const qualification = calculateGroupQualification(matches || [], { events: (redCards || []).filter((event) => playedMatchIds.has(event.match_id)) })
  ...
  const leaderboardSnapshotAfter = await saveLeaderboardSnapshot(supabase, 'after_recompute')
  ...
  return {
    ...
    leaderboard_snapshot: leaderboardSnapshotAfter,
    leaderboard_snapshot_before: leaderboardSnapshotBefore,
