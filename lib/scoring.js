...
export function isGroupStage(stage) {
  return String(stage || '').startsWith('group')
}

export function applyPlayedMatchScore(scores, match) {
...
export function findKnockoutTiesMissingWinner(matches) {
  return (matches || []).filter((match) => {
    if (!match.played || isGroupStage(match.stage)) return false
    return Number(match.home_score || 0) === Number(match.away_score || 0) && !match.winner_team_id
  })
}
