...
export function buildVirtualBracket(matches) {
  ...
  return KNOCKOUT_TEMPLATE.map((template) => {
    ...
    const homeTeamId = existing?.home_team_id || resolveSlot(template.home)
    const awayTeamId = existing?.away_team_id || resolveSlot(template.away)
    return {
      ...template,
      match: existing || null,
...
export async function syncTournamentProgression(supabase, matches) {
  const currentMatches = matches || []
  const virtualBracket = buildVirtualBracket(currentMatches)
