...
export function buildVirtualBracket(matches, options = {}) {
  ...
  function resolveSlot(slot, order) {
    if (/^3[A-L](\/[A-L])+$/.test(slot || '')) {
      ...
      if (!thirdPlaceAssignments) return null
  ...
  function shouldUseExistingTeam(slot) {
    return !(/^3[A-L](\/[A-L])+$/.test(slot || '') && !thirdPlaceAssignments)
  }
  ...
  return KNOCKOUT_TEMPLATE.map((template) => {
    ...
    const homeTeamId = resolvedHomeTeamId || (shouldUseExistingTeam(template.home) ? existing?.home_team_id : null)
    const awayTeamId = resolvedAwayTeamId || (shouldUseExistingTeam(template.away) ? existing?.away_team_id : null)
    return {
      ...template,
      match: existing || null,
