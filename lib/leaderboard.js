...
import { getMatchLoser } from './tournamentProgression'
...
function buildTeamTournamentState(matches) {
  const qualification = calculateGroupQualification(matches || [])
  const eliminated = new Set()

  if (qualification.allGroupsComplete) {
    for (const group of Object.values(qualification.groups || {})) {
      for (const standing of group.table || []) {
        if (!qualification.qualified.has(standing.team_id)) eliminated.add(standing.team_id)
      }
    }
  }

  for (const match of matches || []) {
    if (!match.played || String(match.stage || '').startsWith('group')) continue
    const loserId = getMatchLoser(match)
    if (loserId) eliminated.add(loserId)
  }

  return {
    isEliminated(teamId) {
      return eliminated.has(teamId)
    }
  }
}

function buildPendingScoringState(matches) {
  const currentStage = KNOCKOUT_STAGES.find((stage) =>
    (matches || []).some((match) => match.stage === stage.key && !match.played)
  )
  const pendingTeamIds = new Set()

  if (currentStage) {
    for (const match of matches || []) {
      if (match.stage !== currentStage.key || match.played) continue
      if (match.home_team_id) pendingTeamIds.add(match.home_team_id)
      if (match.away_team_id) pendingTeamIds.add(match.away_team_id)
    }
  }
  ...
  return {
    currentStage,
    isPending(teamId) {
      return pendingTeamIds.has(teamId)
    }
  }
}
...
export async function buildLeaderboard(supabase) {
  const [
    { data: tpoints, error: pointsError },
  ...
  const tournamentState = buildTeamTournamentState(matches || [])
  const pendingScoringState = buildPendingScoringState(matches || [])
  ...
  for (const p of picks || []) {
    ...
    scores[submitter].breakdown.push({
      ...
      eliminated: tournamentState.isEliminated(p.team_id),
      active: !tournamentState.isEliminated(p.team_id),
      pending_current_stage: pendingScoringState.isPending(p.team_id),
      rank: p.rank,
      multiplier: mult,
      team_points: teamPts,
      contributed: add
  ...
  leaderboard.forEach((row, index) => {
    ...
    row.active_teams = row.breakdown.filter((item) => item.active).length
    row.eliminated_teams = row.breakdown.filter((item) => item.eliminated).length
    row.scoring_teams = row.breakdown.filter((item) => Math.abs(Number(item.contributed || 0)) > 0.001).length
    row.pending_scoring_teams = row.breakdown.filter((item) => item.pending_current_stage).length
  })
  ...
  return {
    leaderboard,
    updatedAt,
    pendingScoringStage: pendingScoringState.currentStage
      ? { key: pendingScoringState.currentStage.key, label: pendingScoringState.currentStage.label }
      : null
  }
}

export async function latestLeaderboardSnapshots(supabase, limit = 100) {
